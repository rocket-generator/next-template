...
- [ ] `src/libraries/storage.ts` の既存 `S3Provider` バグ修正（先に済ませる）
  - [ ] `createS3Client()` の endpoint 分岐（現 `storage.ts:380-383`）を書き換え
  ...
  - [ ] `generateFileUrl()` で virtual-hosted-style / path-style の両分岐をテストで検証可能にする
- [ ] `src/libraries/storage.ts` に追加
  - [ ] `createS3CompatibleProviderConfig(): S3ProviderConfig` を export（**既存 `S3ProviderConfig` をそのまま返す**。新 interface は作らない）
    - env:
...
- [ ] `src/libraries/email.ts` に追加
  ...
  - [ ] `createEmailServiceInstance()` を書き換え
    ...
    - [ ] `fromEmail` の解決を provider 非依存に
      - [ ] `process.env.EMAIL_FROM` を優先
      - [ ] なければ `process.env.SES_FROM_EMAIL`（後方互換、`console.warn` でリネーム推奨を出す）
      - [ ] それもなければ環境依存のデフォルト（`NODE_ENV=production` で `noreply@example.com`、それ以外は `noreply@localhost`）
- [ ] `package.json` の `dependencies` に `nodemailer` を追加
- [ ] `package.json` の `devDependencies` に `@types/nodemailer` を追加

### env / ドキュメント

- [ ] `.env.example` を以下の方針で書き換え
  - [ ] **前提認識（事実ベース）**: 現行 `.env.example`（git HEAD 時点）には以下が存在する
  ...
  - [ ] **runtime と乖離している箇所**（本タスクで解消）
    - [ ] `SYSTEM_AWS_S3_REGION` が `.env.example` に未記載（`storage.ts:512` が参照）
    - [ ] `LOCALSTACK_PUBLIC_ENDPOINT` が未記載（`storage.ts:518` が参照。LocalStack 利用時の公開 URL 生成で使用）
  - [ ] **欠落キーの新規追加**
    - [ ] `SYSTEM_AWS_S3_REGION=us-east-1`（AWS SES Configuration ブロック内、他 `SYSTEM_AWS_*` と同じ場所）
    - [ ] `LOCALSTACK_PUBLIC_ENDPOINT=http://localhost:4566`（LocalStack Configuration ブロック内）
  - [ ] **既存 `SYSTEM_AWS_*` エントリは保持**（削除や rename はしない）
  - [ ] **`SES_FROM_EMAIL` は deprecated 扱いで残す**
    - [ ] `EMAIL_FROM=noreply@yourdomain.com` を新規追加（`SES_FROM_EMAIL` の直前に置く）
    - [ ] `SES_FROM_EMAIL` の行に `# deprecated: use EMAIL_FROM (kept for backward compatibility)` コメントを付与
  - [ ] Storage プロバイダ切替ブロックを新規追加
...
- [ ] **`docker-compose.yml` の env を `SYSTEM_AWS_*` に更新**（line 65-72）
  - [ ] `AWS_S3_REGION` → `SYSTEM_AWS_S3_REGION`
  - [ ] `AWS_ACCESS_KEY_ID` → `SYSTEM_AWS_ACCESS_KEY_ID`
  - [ ] `AWS_SECRET_ACCESS_KEY` → `SYSTEM_AWS_SECRET_ACCESS_KEY`
  - [ ] `AWS_S3_BUCKET` → `SYSTEM_AWS_S3_BUCKET`
  - [ ] `AWS_SES_REGION` → `SYSTEM_AWS_SES_REGION`
  - [ ] `SES_FROM_EMAIL` → `EMAIL_FROM`（旧名を併記しておくかは実装時判断。互換フォールバック有）
  - [ ] コメント「# LocalStack S3 Configuration」を「# LocalStack S3 / SES Configuration (SYSTEM_ prefix required for Amplify compatibility)」に更新
- [ ] **`README.md:196-202` の env 一覧を更新**
  - [ ] `AWS_S3_REGION` / `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / `AWS_S3_BUCKET` / `AWS_SES_REGION` を `SYSTEM_AWS_*` に
  - [ ] `SES_FROM_EMAIL` → `EMAIL_FROM` に（互換フォールバックの旨コメント）
  - [ ] `STORAGE_PROVIDER` / `EMAIL_PROVIDER` / `S3_COMPATIBLE_*` / `SMTP_*` / `GCS_*` の説明を追記
...
- [ ] `__tests__/libraries/storage.test.ts` を拡充
  - [ ] `STORAGE_PROVIDER` 未設定で S3Provider が使われる（`SYSTEM_AWS_*` env 経由で config が組まれる）
  - [ ] `s3-compatible` で endpoint / publicEndpoint / forcePathStyle が S3Provider に正しく渡る
    - [ ] `S3_COMPATIBLE_FORCE_PATH_STYLE=false` で `false` が実際に S3Client config に載ること（既存バグの回帰防止）
    - [ ] `S3_COMPATIBLE_PUBLIC_ENDPOINT` 未設定時、`endpoint` と同値になること
    - [ ] `S3_COMPATIBLE_ENDPOINT` 未設定で throw
  - [ ] `generateFileUrl()` が `forcePathStyle: true` と `false` で URL 形が正しく切り替わる
...
- [ ] `__tests__/libraries/email.test.ts` を拡充
  ...
  - [ ] `EMAIL_FROM` が設定されているとそれが使われる
  - [ ] `EMAIL_FROM` 未設定で `SES_FROM_EMAIL` のみ設定されているときは fallback し、`console.warn` が呼ばれる
  - [ ] 両方未設定で環境依存デフォルトになる
- [ ] **consumer 側の smoke test を追加**
  - [ ] `__tests__/repositories/user_repository.test.ts`（or 新規）で `new UserRepository()` が `STORAGE_PROVIDER=s3-compatible` でも例外を投げずに構築できる（`@aws-sdk/client-s3` は `jest.mock`）
  - [ ] `__tests__/libraries/auth.test.ts` で `EMAIL_PROVIDER=smtp` 切替時も Better Auth の sendResetPassword / sendVerificationEmail コールバックが例外を出さずに SMTPProvider.sendEmail を呼ぶ
- [ ] `npm run test` 全 pass
...
- [ ] `docker-compose.yml` の env ブロック更新後、ローカル開発者は既存 `.env` を見直す必要あり。移行手順を README に記載
- [ ] `SES_FROM_EMAIL` は `EMAIL_FROM` に推奨移行。旧名は後方互換として残すが `console.warn` が出るため、派生プロジェクト側で順次更新
- [ ] `STORAGE_PROVIDER` / `EMAIL_PROVIDER` 未設定時は既定 `s3` / `ses` なので既存動作を壊さない
- [ ] `forcePathStyle` バグ修正により、`endpoint` 指定 + 明示的に `forcePathStyle: false` を渡していた既存プロジェクトが（もしあれば）動作変化する可能性。ただし現状はバグで常に true だったので実質的な影響はほぼ無し
...
- [ ] `.env.example` / `docker-compose.yml` / `README.md` が実装と一致（3 ファイル揃って `SYSTEM_AWS_*` / `EMAIL_FROM`）
